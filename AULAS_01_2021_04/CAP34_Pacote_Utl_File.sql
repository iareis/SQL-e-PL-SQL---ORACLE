...
O pacote UTL_FILE é um conjunto de componentes que tem como função permitir o acesso
ou geração de arquivos externos ao banco de dados.

Um exemplo seria importarmos scripts em SQL ou PL/SQL para o nosso sistema.

O pacote é constituído de 18 componentes, são eles:
Exceptions, Functions, Procedures e Types.

Para utilizarmos o pacote, teremos que interromper os serviços do banco de dados
e modificar o arquivo initXE.ora (ORACLE 11G Express Edition),
onde iremos acrescentar o parâmetro UTL_FILE_DIR, afim de determinar quais os diretórios
do sistema está livre para o acesso.

Abra o CMD do windows ou terminal Linux e vamos informar ao banco de dados
que podemos gravar e recuperar dados do sistema operacional, através dos seguintes passos:
...
FOPEN
abre um arquivo para entrada ou saída, criando um arquivo de saída caso o arquivo
especificado não exista
...
NO_DATA_FOUND
nesse caso, é disparada quando o fim do arquivo é encontrado em processamento de
leitura seqüencial de um arquivo de texto
...
BEGIN
 arquivo_saida := UTL_File.Fopen('DIRETORIO','Lista.txt','w'); --w write
